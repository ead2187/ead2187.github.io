...
var NUMSINES; // how many of these things can we do at once? map to minutes
var sines; // an array to hold all the current angles
...
function setup() {
  ...
  var h = hour ();
  rad = map(h, 0, 24, height/7, height/7); // compute radius for central circle HOUR
  background(255, 87, 51); // clear the screen
...
function draw() {
  ...
  if (!trace) {
    background(255, 87, 51); // clear screen if showing geometry
    stroke(51,115,255); // blue pen
    noFill(); // don't fill
  } 
